...
def patch_database_registry() -> None:
    path = ROOT / "backend/app/database.py"
    text = path.read_text(encoding="utf-8")
    if '    "driver_fee_ledger",\n' in text:
        return
    anchor = '    "hailing_trip_events",\n'
    if anchor not in text:
        raise RuntimeError("database collection registry anchor missing")
    text = text.replace(
        anchor,
        anchor
        + '    "driver_fee_ledger",\n'
...
def rewrite_finance_contract_test() -> None:
    path = ROOT / "backend/tests/test_driver_cash_card_finance.py"
    path.write_text(
        '''from __future__ import annotations
...
def main() -> None:
    patch_database_registry()
    rewrite_finance_contract_test()
    print("Weekly settlement validation contract aligned")
