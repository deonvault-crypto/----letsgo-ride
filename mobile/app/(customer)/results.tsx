...
export default function ResultsScreen() {
  ...
  return (
    <Screen title="Results" showBack fallbackRoute="/(customer)/search" navRole="customer">
      ...
      {!loading && !error && availableRides.length === 0 ? <EmptyState title="No rides yet" body="Try a nearby city, a later date, or fewer seats." icon="map-search-outline" /> : null}
      {!loading && !error && availableRides.map((ride) => <RideCard key={ride.id} ride={ride} onPress={() => router.push(`/(customer)/ride/${ride.id}` as never)} onDriverPress={() => openDriverProfile(ride.driver_id, ride.id)} />)}
    </Screen>
  );
}

const styles = StyleSheet.create({
  title: { color: colors.whiteText, fontWeight: "900", fontSize: 28 },
  body: { color: colors.mutedText, lineHeight: 22, marginBottom: spacing.sm },
});
