...
export default function CourierHomeScreen() {
  ...
  const photoRequired = Boolean(profile && approved && !hasProfilePhoto && !profile.online);

  async function toggleOnline() {
    if (!profile || busy) return;
    if (!profile.online && !hasProfilePhoto) {
      setError("Add a clear profile photo before going online for new deliveries.");
      return;
    }
    try {
      setBusy(true);
      ...
      {active ? (
        ...
        <>
          <View style={styles.idleHero}>
            ...
            <Text style={styles.title}>{photoRequired ? "Add your profile photo." : online ? "You’re ready for nearby work." : "Start when you’re ready."}</Text>
            <Text style={styles.darkBody}>{approved ? (photoRequired ? "A clear Courier profile photo is required before you can go online for new deliveries." : online ? `${offerCount} available ${offerCount === 1 ? "offer" : "offers"} nearby. You can accept one job at a time.` : "Go online to receive real Courier and Food delivery offers.") : "Your application must be approved before work access opens."}</Text>
            {profile ? (
              photoRequired ? (
                <Pressable
                  accessibilityRole="button"
                  accessibilityLabel="Add required Courier profile photo"
                  onPress={() => router.push({ pathname: "/(shared)/edit-profile", params: { product: "courier" } } as never)}
                  style={({ pressed }) => [styles.photoButton, pressed && styles.pressed]}
                >
                  <MaterialCommunityIcons name="camera-plus-outline" size={21} color="#FFFFFF" />
