...
export default function FoodCheckoutScreen() {
  ...
  async function placeOrder() {
    ...
    try {
      ...
      const order = await createFoodOrder({
        ...
        delivery_location: { latitude: foodDropoff.location.latitude, longitude: foodDropoff.location.longitude },
  ...
  if (!basket.restaurant || basket.lines.length === 0) {
    return (
      <Screen showBack fallbackRoute="/(customer)/food" title="Checkout" showNotifications={false}>
  ...
  return (
    <Screen showBack fallbackRoute={`/(shared)/food/${basket.restaurant.id}` as never} title="Checkout" showNotifications={false}>
      ...
      <View style={styles.summaryCard}>
        ...
        <View style={styles.securityNote}><MaterialCommunityIcons name="shield-check-outline" size={18} color={v2Theme.colors.brandStrong} /><Text style={styles.securityText}>Prices, item availability and delivery pricing are checked again by the server before the order starts.</Text></View>
      </View>
