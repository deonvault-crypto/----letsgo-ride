...
export default function ActivityScreen() {
  ...
  const loadServices = useCallback(async () => {
    try {
      ...
      const [orders, deliveries] = await Promise.all([listMyFoodOrders(), listMyCourierDeliveries()]);
  ...
  const timeline = useMemo<TimelineItem[]>(() => {
    ...
    return items.filter((item) => filter === "all" || (filter === "rides" && item.kind === "ride") || (filter === "food" && item.kind === "food") || (filter === "courier" && item.kind === "courier")).sort((a, b) => new Date(b.createdAt || 0).getTime() - new Date(a.createdAt || 0).getTime());
  }, [trips, foodOrders, courierDeliveries, filter]);

  const loading = ridesLoading || servicesLoading;
  const error = ridesError || servicesError;
  async function retry() { await Promise.all([reloadRides(), loadServices()]); }

  return (
    <Screen navRole="customer">
      <View style={styles.hero}><Text style={styles.eyebrow}>YOUR LETSGORIDE</Text><Text style={styles.title}>Activity</Text><Text style={styles.body}>Every ride, food order and courier delivery in one clean timeline.</Text></View>
      ...
      {!loading && !error && timeline.length === 0 ? <View style={styles.emptyCard}><View style={styles.emptyIcon}><MaterialCommunityIcons name="history" size={29} color={v2Theme.colors.brandStrong} /></View><Text style={styles.emptyTitle}>Nothing here yet</Text><Text style={styles.emptyBody}>Your LetsGoRide activity will appear here as soon as you reserve a ride, order food or send a delivery.</Text><Pressable accessibilityRole="button" onPress={() => router.push("/(shared)/services" as never)} hitSlop={8}><Text style={styles.action}>Explore services</Text></Pressable></View> : null}
...
function RideActivityCard({ trip, onPress }: { trip: RideRequest; onPress: () => void }) { const ride = trip.ride_snapshot; return <ActivityCard icon="car-outline" iconTone="neutral" service="RIDE" title={ride ? `${ride.origin} → ${ride.destination}` : "Ride details"} subtitle={ride ? [ride.date, ride.time].filter(Boolean).join(" · ") : "Open trip for details"} status={friendlyStatus(trip.status)} trailing={ride?.price_usd != null ? `$${ride.price_usd.toFixed(2)}` : undefined} onPress={onPress} />; }
...
function CourierActivityCard({ delivery, onPress }: { delivery: CourierDelivery; onPress: () => void }) { return <ActivityCard icon="package-variant-closed" iconTone="brand" service="COURIER" title={`${delivery.pickup_address} → ${delivery.dropoff_address}`} subtitle={`${delivery.package_type.replaceAll("_", " ")} · ${formatDate(delivery.created_at)}`} status={friendlyStatus(delivery.status)} trailing={delivery.price_usd != null ? `$${delivery.price_usd.toFixed(2)}` : undefined} onPress={onPress} />; }
