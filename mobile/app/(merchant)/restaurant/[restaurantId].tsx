...
import { FoodOrder, MenuItem } from "../../../types/food.types";
...
export default function MerchantRestaurantScreen() {
  ...
  const load = useCallback(async (quiet = false) => {
    ...
    try {
      ...
      if (!quiet) setError(err instanceof Error ? err.message : "Unable to load restaurant workspace.");
    } finally {
  ...
  async function toggleItem(item: MenuItem) {
    ...
    try {
      setBusy(true);
      setError(null);
      const updated = await updateMenuItem(item.id, { is_available: item.is_available === false });
      setWorkspace({ ...workspace, items: workspace.items.map((entry) => entry.id === updated.id ? updated : entry) });
    } catch (err) {
      setError(err instanceof Error ? err.message : "Unable to change item availability.");
    } finally {
      setBusy(false);
    }
  }
        ...
        <>
          <View style={styles.heroCard}>
            ...
            <View style={styles.metricsRow}>
              ...
              <Metric label="Available items" value={String(workspace.items.filter((item) => item.is_available !== false).length)} />
          ...
          <View style={styles.controlCard}>
            ...
            <Pressable accessibilityRole="button" accessibilityState={{ disabled: workspace.restaurant.status !== "ACTIVE" || busy }} disabled={workspace.restaurant.status !== "ACTIVE" || busy} onPress={toggleAccepting} style={[styles.toggleButton, workspace.restaurant.is_accepting_orders && styles.toggleButtonOn, workspace.restaurant.status !== "ACTIVE" && styles.disabled]}>
          ...
          <View style={styles.section}>
            <View style={styles.sectionHeader}><View><Text style={styles.sectionTitle}>Kitchen orders</Text><Text style={styles.sectionSub}>Orders arrive here automatically</Text></View><Text style={styles.count}>{activeOrders.length}</Text></View>
            ...
            <View style={styles.orderList}>{activeOrders.map((order) => <MerchantOrderCard key={order.id} order={order} busy={busy} onReady={markReady} />)}</View>
          </View>

          <View style={styles.section}>
            <View style={styles.sectionHeader}><View><Text style={styles.sectionTitle}>Menu</Text><Text style={styles.sectionSub}>Switch sold-out items off instantly</Text></View><Text style={styles.count}>{workspace.items.length}</Text></View>
            ...
            <View style={styles.builderCard}>
              ...
              {workspace.categories.length ? <View style={styles.categoryWrap}>{workspace.categories.map((category) => <Pressable key={category.id} accessibilityRole="button" accessibilityState={{ selected: selectedCategory === category.id }} onPress={() => setSelectedCategory(category.id)} style={[styles.categoryPill, selectedCategory === category.id && styles.categoryPillActive]}><Text style={[styles.categoryText, selectedCategory === category.id && styles.categoryTextActive]}>{category.name}</Text></Pressable>)}</View> : null}
            </View>
            ...
            <View style={styles.builderCard}>
              ...
              <Pressable accessibilityRole="button" onPress={addItem} disabled={!selectedCategory || !itemName.trim() || !itemPrice.trim() || busy} style={[styles.addItemButton, (!selectedCategory || !itemName.trim() || !itemPrice.trim() || busy) && styles.disabled]}><Text style={styles.addItemText}>Add to menu</Text><MaterialCommunityIcons name="plus" size={20} color="#FFFFFF" /></Pressable>
            </View>

            <View style={styles.menuList}>
              {workspace.items.map((item) => {
                const available = item.is_available !== false;
                return <View key={item.id} style={[styles.menuItemRow, !available && styles.menuItemUnavailable]}>
                  <View style={styles.menuItemIcon}><MaterialCommunityIcons name="food-outline" size={22} color={v2Theme.colors.ink} /></View>
                  <View style={styles.menuItemCopy}><Text style={styles.menuItemName}>{item.name}</Text><Text style={styles.menuItemMeta}>${item.price_usd.toFixed(2)} · {available ? "Available" : "Sold out"}{item.preparation_minutes != null ? ` · ${item.preparation_minutes}m` : ""}</Text></View>
                  <Pressable accessibilityRole="switch" accessibilityState={{ checked: available, disabled: busy }} accessibilityLabel={`${item.name} availability`} disabled={busy} onPress={() => toggleItem(item)} style={[styles.itemToggle, available && styles.itemToggleOn, busy && styles.disabled]}><View style={[styles.itemToggleDot, available && styles.itemToggleDotOn]} /></Pressable>
                </View>;
              })}
            </View>
          </View>
...
function MerchantOrderCard({ order, busy, onReady }: { order: FoodOrder; busy: boolean; onReady: (order: FoodOrder) => void }) {
  ...
  return <View style={styles.orderCard}>
    <View style={styles.orderTop}><View><Text style={styles.orderId}>ORDER {order.id.slice(0, 7).toUpperCase()}</Text><Text style={styles.orderName}>{order.recipient_name}</Text></View><View style={[styles.orderStatusPill, isReady && styles.orderStatusReady]}><Text style={[styles.orderStatus, isReady && styles.orderStatusReadyText]}>{isReady ? "READY" : "PREPARING"}</Text></View></View>
    ...
    <View style={styles.dispatchStrip}><MaterialCommunityIcons name="motorbike" size={18} color={v2Theme.colors.brandStrong} /><Text style={styles.dispatchText}>{fulfillmentLabel(order.fulfillment_status)}</Text></View>
    {isPreparing ? <Pressable disabled={busy} onPress={() => onReady(order)} style={[styles.readyButton, busy && styles.disabled]}><Text style={styles.readyText}>Order is ready for pickup</Text><MaterialCommunityIcons name="check" size={19} color="#FFFFFF" /></Pressable> : null}
  </View>;
}
...
function EmptyRow({ icon, title, body }: { icon: keyof typeof MaterialCommunityIcons.glyphMap; title: string; body: string }) { return <View style={styles.emptyRow}><View style={styles.emptyIcon}><MaterialCommunityIcons name={icon} size={23} color={v2Theme.colors.inkSecondary} /></View><View style={styles.emptyCopy}><Text style={styles.emptyTitle}>{title}</Text><Text style={styles.emptyBody}>{body}</Text></View></View>; }

const styles = StyleSheet.create({
  loading: { color: v2Theme.colors.inkSecondary, fontSize: 12 }, errorCard: { minHeight: 58, borderRadius: v2Theme.radius.lg, backgroundColor: v2Theme.colors.dangerSoft, padding: 12, flexDirection: "row", alignItems: "center", gap: 9 }, errorText: { flex: 1, color: v2Theme.colors.danger, fontSize: 11, fontWeight: "700" }, retry: { color: v2Theme.colors.danger, fontSize: 11, fontWeight: "900" },
  heroCard: { borderRadius: v2Theme.radius.xxl, backgroundColor: v2Theme.colors.ink, padding: 17, gap: 15 }, heroTop: { flexDirection: "row", alignItems: "center", gap: 12 }, heroIcon: { width: 52, height: 52, borderRadius: 18, backgroundColor: "#FFFFFF", alignItems: "center", justifyContent: "center" }, heroCopy: { flex: 1, gap: 4 }, heroEyebrow: { color: "rgba(255,255,255,0.6)", fontSize: 8, fontWeight: "900", letterSpacing: 0.9 }, heroTitle: { color: "#FFFFFF", fontSize: 20, fontWeight: "900" }, heroAddress: { color: "rgba(255,255,255,0.66)", fontSize: 10 }, metricsRow: { flexDirection: "row", gap: 8 }, metric: { flex: 1, borderRadius: 16, backgroundColor: "rgba(255,255,255,0.08)", padding: 11, gap: 3 }, metricValue: { color: "#FFFFFF", fontSize: 18, fontWeight: "900" }, metricLabel: { color: "rgba(255,255,255,0.56)", fontSize: 8, fontWeight: "800" },
  controlCard: { minHeight: 78, borderRadius: v2Theme.radius.xl, backgroundColor: v2Theme.colors.surface, borderWidth: StyleSheet.hairlineWidth, borderColor: v2Theme.colors.line, padding: 14, flexDirection: "row", alignItems: "center", gap: 12 }, controlCopy: { flex: 1, gap: 4 }, controlTitle: { color: v2Theme.colors.ink, fontSize: 14, fontWeight: "900" }, controlBody: { color: v2Theme.colors.inkSecondary, fontSize: 10, lineHeight: 15 }, toggleButton: { minHeight: 38, borderRadius: v2Theme.radius.pill, backgroundColor: v2Theme.colors.surfaceMuted, paddingHorizontal: 10, flexDirection: "row", alignItems: "center", gap: 6 }, toggleButtonOn: { backgroundColor: v2Theme.colors.brand }, toggleDot: { width: 7, height: 7, borderRadius: 4, backgroundColor: v2Theme.colors.inkTertiary }, toggleDotOn: { backgroundColor: "#FFFFFF" }, toggleText: { color: v2Theme.colors.ink, fontSize: 10, fontWeight: "900" }, toggleTextOn: { color: "#FFFFFF" },
