...
export default function DriverAccountScreen() {
  ...
  useFocusEffect(useCallback(() => {
    let active = true;
    void listMyWorkerApplications().then((items) => { if (active) setApplication(items.find((item) => item.product === "driver") || null); }).catch(() => undefined);
    return () => { active = false; };
  }, []));
  return (
    <Screen title="Account" navRole="driver">
      ...
      <Section title="Driver profile">
        ...
        <Row icon="star-outline" title="Rating" value={driver?.rating ? `${Number(driver.rating).toFixed(1)} ⭐` : "NEW · No reviews yet"} />
      </Section>
      <Section title="Earnings & payouts">
        <Pressable accessibilityRole="button" onPress={() => router.push("/(shared)/wallet" as never)}><Row icon="wallet-outline" title="Wallet" value="Balance, cash ledger and payout methods" chevron /></Pressable>
      </Section>
      ...
      <Section title="Settings"><Pressable accessibilityRole="button" onPress={() => router.push({ pathname: "/(shared)/settings", params: productParam } as never)}><Row icon="cog-outline" title="Account & notifications" value="Privacy, account and alerts" chevron /></Pressable></Section>
...
const styles = StyleSheet.create({ flex: { flex: 1 }, profile: { flexDirection: "row", alignItems: "center", gap: 13 }, name: { color: v2Theme.colors.ink, fontSize: 23, fontWeight: "900" }, meta: { color: v2Theme.colors.inkSecondary, fontSize: 10, marginTop: 3 }, verification: { borderRadius: 23, backgroundColor: v2Theme.colors.warningSoft, padding: 14, flexDirection: "row", alignItems: "center", gap: 11 }, verificationGood: { backgroundColor: v2Theme.colors.brandSofter }, verificationIcon: { width: 49, height: 49, borderRadius: 17, backgroundColor: v2Theme.colors.surface, alignItems: "center", justifyContent: "center" }, verificationTitle: { color: v2Theme.colors.ink, fontSize: 14, fontWeight: "900" }, verificationBody: { color: v2Theme.colors.inkSecondary, fontSize: 9, lineHeight: 14, marginTop: 3 }, section: { gap: 8 }, sectionTitle: { color: v2Theme.colors.ink, fontSize: 18, fontWeight: "900" }, card: { borderRadius: 23, backgroundColor: v2Theme.colors.surface, overflow: "hidden" }, row: { minHeight: 69, paddingHorizontal: 12, flexDirection: "row", alignItems: "center", gap: 10, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: v2Theme.colors.line }, rowIcon: { width: 41, height: 41, borderRadius: 14, backgroundColor: v2Theme.colors.surfaceMuted, alignItems: "center", justifyContent: "center" }, rowTitle: { color: v2Theme.colors.ink, fontSize: 11, fontWeight: "900" }, rowValue: { color: v2Theme.colors.inkSecondary, fontSize: 9, marginTop: 2 } });
