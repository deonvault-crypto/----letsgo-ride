...
describe("Driver workspace realtime", () => {
  ...
  it("loads one shared snapshot and removes Driver Home and Trips timer polling", async () => {
    ...
    const requestsHook = fs.readFileSync(path.join(__dirname, "../hooks/useDriverRequests.ts"), "utf8");
    ...
    expect(requestsHook).not.toContain("useLiveRefresh");
    expect(ridesHook).toContain("useDriverWorkspace");
    expect(requestsHook).toContain("useDriverWorkspace");
