...
import RequestSeatScreen from "../app/(passenger)/request/[id]";
import MyTripsScreen from "../app/(passenger)/my-trips";
...
describe("ride booking flow", () => {
  ...
  it("requires a phone number before booking a seat", async () => {
    ...
    expect(screen.getByText("Passengers and drivers need a reachable number for pickup coordination and trip safety.")).toBeOnTheScreen();
  ...
  it("sends a booking request and shows confirmation", async () => {
    ...
    await waitFor(() => {
      expect(requestSeat).toHaveBeenCalledWith({
        ride_id: "ride-1",
  ...
  it("saves phone from the completion modal and shows booked rides in trips", async () => {
    ...
    await waitFor(() => {
      expect(updateCurrentUser).toHaveBeenCalledWith({ phone: "+263778888888" });
    });

    mockPathname = "/my-trips";
    const tripsScreen = render(<MyTripsScreen />);
