...
import SearchRideScreen from "../app/(passenger)/search";
import ResultsScreen from "../app/(passenger)/results";
...
jest.mock("../hooks/useRides", () => ({
  useRides: jest.fn(() => mockRidesState),
}));
...
describe("ride search flow", () => {
  ...
  it("displays ride results and a clean empty state", async () => {
    ...
    expect(useRides).toHaveBeenCalledWith({
      origin: "Harare",
