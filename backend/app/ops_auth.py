...
def effective_ops_role(user: Dict[str, Any]) -> str:
    # Preserve the existing mobile/product role model. Existing admins retain
    # full operations access, while CS/Manager privileges live in ops_role.
...
def can_manage_case_level(user: Dict[str, Any], escalation_level: str) -> bool:
    role = effective_ops_role(user)
    required = {"cs": 10, "manager": 20, "admin": 30}.get(escalation_level, 30)
