...
FINAL_HAILING_STATUSES = {
    "COMPLETED", "CANCELLED", "CANCELLED_BY_PASSENGER", "CANCELLED_BY_DRIVER", "CANCELLED_BY_ADMIN", "EXPIRED", "NO_DRIVER_FOUND",
    "completed", "cancelled", "cancelled_by_passenger", "cancelled_by_driver", "cancelled_by_admin", "expired", "no_driver_found",
}
...
def _safe_source(source_type: str, source: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    if not source:
        return None
    if source_type == "support_message":
        return _safe_support(source)
    if source_type == "safety_report":
        return _safe_report(source)
    if source_type == "shared_ride":
        return _safe_ride(source)
    if source_type == "ride_request":
        return _safe_ride_request(source)
    if source_type == "hailing_trip":
        return _safe_hailing(source)
    if source_type == "courier_delivery":
        return _safe_courier(source)
    if source_type == "food_order":
        return _safe_food(source)
    if source_type == "user":
...
async def _source_record(source_type: str, source_id: Optional[str]) -> Optional[Dict[str, Any]]:
    ...
    if not collection or not source_id:
        api_error("A valid source ID is required for this case type.", 400)
...
def _source_customer_user_id(source_type: str, source: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    if source_type in {"support_message", "safety_report", "ride_request"}:
...
async def _case_event(
    ...
    *,
    ...
    await write_audit_log(
        actor_user_id=actor.get("id"),
        actor_role=f"ops:{effective_ops_role(actor)}",
...
async def live_operations(user=Depends(get_ops_user)):
    ...
    return api_success({
        ...
        "courier": [_safe_courier(row) for row in courier],
        "food": [_safe_food(row) for row in food],
        "shared_rides": [_safe_ride(row) for row in rides],
    })
...
async def update_support_message(
    ...
    if updated.get("user_id"):
        await create_app_notification(
            ...
            "LetsGoRide Support updated your case. Open the app for details.",
    ...
    await write_audit_log(
        ...
        metadata={"status": payload.status, "reply_added": bool(payload.reply)},
    )
    return api_success(_safe_support(updated))
...
async def update_safety_report(
    ...
    await write_audit_log(
        ...
        target_type="report",
        target_id=report_id,
        metadata={"status": payload.status, "notes_added": bool(payload.notes)},
    )
    return api_success(_safe_report(updated))
...
async def add_case_note(
    ...
    if not case:
        api_error("Operations case not found.", 404)
    if not can_manage_case_level(user, _case_level(case)):
