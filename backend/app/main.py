...
from app.services.communications_service import ensure_communications_indexes
...
background_workers_stop_event: asyncio.Event | None = None
background_workers_task: asyncio.Task | None = None
...
async def on_startup():
    global background_workers_stop_event, background_workers_task
    ...
    # Every web instance serves HTTP/realtime traffic, but only the instance that
    # owns the distributed Mongo lease runs periodic background workers. Followers
    # remain hot and can take over after lease expiry.
    background_workers_stop_event = asyncio.Event()
    background_workers_task = asyncio.create_task(
        background_worker_supervisor(background_workers_stop_event),
        name="letsgoride:background-worker-supervisor",
    )
    logger.info("background_worker_supervisor_started")


@app.on_event("shutdown")
async def on_shutdown():
    global background_workers_stop_event, background_workers_task
    if background_workers_stop_event:
        background_workers_stop_event.set()
    if background_workers_task:
        try:
            await asyncio.wait_for(background_workers_task, timeout=8)
        except asyncio.TimeoutError:
            background_workers_task.cancel()
            await asyncio.gather(background_workers_task, return_exceptions=True)
