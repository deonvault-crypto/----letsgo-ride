...
async def list_conversations_for_user_scaled(user: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if user.get("role") == "admin":
        conversations = await find_many_bounded(
            "conversations",
            {},
            sort=[("updated_at", -1)],
        ...
        by_id = {str(item.get("id")): item for item in [*driver_rows, *passenger_rows] if item.get("id")}
...
async def mark_conversation_read_scaled(conversation: Dict[str, Any], user: Dict[str, Any]) -> Dict[str, Any]:
    """Mark unread messages without materializing the full unread message history."""
    user_id = str(user.get("id") or "")
    ...
    latest_unread = await find_one_sorted(
        "messages",
        filters,
        sort=[("created_at", -1)],
    )
    if not latest_unread:
        return {
            "read": True,
            "changed": 0,
            "conversation_realtime_version": conversation_realtime_version(conversation),
    ...
    read_through_message_id = str(latest_unread.get("id") or "")
