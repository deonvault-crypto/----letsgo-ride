...
function badge(value) {
  ...
  return `<span class="badge ${tone} ${esc(normalized)}">${esc(normalized.replaceAll('_', ' '))}</span>`;
}
...
$('#refreshBtn').addEventListener('click', () => openView(state.currentView, true));
...
async function openView(name, force = false) {
  ...
  try {
    ...
    if (name === 'cases') await renderCases();
    if (name === 'support') await renderSupport();
    if (name === 'users') await renderUsers();
    if (name === 'safety') await renderSafety();
    if (name === 'verifications') await renderVerifications();
    if (name === 'staff') await renderStaff(force);
  ...
  $('#overviewView').innerHTML = `
    ...
    <div class="panel-grid">
      ...
      <section class="panel">
        ...
        <div class="panel-body"><div class="metrics" style="grid-template-columns:1fr 1fr">${metric('Manager', data.manager_escalations)}${metric('Admin', data.admin_escalations)}</div></div>
      </section>
    </div>`;
...
function liveBlock(title, rows, type) {
  const items = rows?.length ? rows.map(row => {
    const route = row.origin && row.destination ? `${row.origin} → ${row.destination}` : row.pickup_address && row.dropoff_address ? `${row.pickup_address} → ${row.dropoff_address}` : row.restaurant_id || row.city_id || row.id;
...
async function renderCases() {
  $('#casesView').innerHTML = `<section class="panel"><div class="panel-head"><div><h2>Operations cases</h2><p>One accountable queue with enforced escalation.</p></div><div class="toolbar"><button id="newCaseBtn" class="primary">New case</button></div></div><div class="panel-body"><div class="toolbar"><input id="caseSearch" placeholder="Search case, issue or assignee"><select id="caseLevel"><option value="">All levels</option><option value="cs">CS</option><option value="manager">Manager</option><option value="admin">Admin</option></select><select id="caseStatus"><option value="">All statuses</option><option value="open">Open</option><option value="in_progress">In progress</option><option value="waiting_customer">Waiting customer</option><option value="escalated">Escalated</option><option value="resolved">Resolved</option><option value="closed">Closed</option></select></div><div id="casesTable" style="margin-top:14px"></div></div></section>`;
...
function openCreateCase(seed = {}) {
  ...
  $('#newCaseForm').onsubmit = async event => {
    ...
    try {
      ...
      const created = await request('/ops/cases', { method: 'POST', body: JSON.stringify({ subject: $('#newCaseSubject').value.trim(), description: $('#newCaseDescription').value.trim(), priority: $('#newCasePriority').value, source_type: sourceType, source_id: sourceId || null }) });
      $('#actionDialog').close();
...
async function openCase(caseId) {
  ...
  $('#addCaseNote').onclick = async () => {
    ...
    try {
      await request(`/ops/cases/${encodeURIComponent(caseId)}/notes`, { method: 'POST', body: JSON.stringify({ note }) });
      toast('Note added.'); $('#caseDialog').close(); await openCase(caseId);
    } catch (error) { toast(error.message, true); }
  };
  ...
  $('#assignCase').onclick = async () => {
    try {
      if (state.me.ops_role === 'cs') {
        await request(`/ops/cases/${encodeURIComponent(caseId)}/assign`, { method: 'PATCH', body: JSON.stringify({ assigned_user_id: state.me.id }) });
      } else {
      ...
      toast('Case assigned.'); $('#caseDialog').close(); await openCase(caseId);
    } catch (error) { toast(error.message, true); }
  };

  $('#statusCase').onclick = () => openStatusDialog(caseId, c.status);
  if ($('#escalateCase')) $('#escalateCase').onclick = () => openEscalationDialog(caseId, nextLevel);
}
...
function openAssignmentDialog(caseId, staff) {
  ...
  $('#assignForm').onsubmit = async event => {
    event.preventDefault();
    try {
      await request(`/ops/cases/${encodeURIComponent(caseId)}/assign`, { method: 'PATCH', body: JSON.stringify({ assigned_user_id: $('#assignUser').value || null }) });
      $('#actionDialog').close(); $('#caseDialog').close(); toast('Assignment updated.'); await openCase(caseId);
    } catch (error) { toast(error.message, true); }
  };
}
...
function openStatusDialog(caseId, currentStatus) {
  ...
  $('#statusForm').onsubmit = async event => {
    event.preventDefault();
    try {
      await request(`/ops/cases/${encodeURIComponent(caseId)}/status`, { method: 'PATCH', body: JSON.stringify({ status: $('#newStatus').value, note: $('#statusNote').value.trim() || null }) });
      $('#actionDialog').close(); $('#caseDialog').close(); toast('Case status updated.'); await openCase(caseId);
    } catch (error) { toast(error.message, true); }
  };
}
...
function openEscalationDialog(caseId, nextLevel) {
  ...
  $('#escalateForm').onsubmit = async event => {
    event.preventDefault();
    try {
      await request(`/ops/cases/${encodeURIComponent(caseId)}/escalate`, { method: 'POST', body: JSON.stringify({ reason: $('#escalateReason').value.trim() }) });
      $('#actionDialog').close(); $('#caseDialog').close(); toast(`Case escalated to ${nextLevel}.`); await openView('cases');
    } catch (error) { toast(error.message, true); }
  };
}
...
async function renderSupport() {
  ...
  $('#supportTable').onclick = event => {
    ...
    if (makeCase) openCreateCase({ source_type: 'support_message', source_id: row.id, subject: row.subject || 'Customer support case', description: row.message || 'Support request' });
  };
  await load();
}
...
function openSupportAction(row) {
  ...
  $('#supportActionForm').onsubmit = async event => {
    event.preventDefault();
    try {
      await request(`/ops/support/messages/${encodeURIComponent(row.id)}`, { method: 'PATCH', body: JSON.stringify({ status: $('#supportActionStatus').value, reply: $('#supportReply').value.trim() || null }) });
      $('#actionDialog').close(); toast('Support message updated.'); await renderSupport();
    } catch (error) { toast(error.message, true); }
  };
}
...
async function renderSafety() {
  ...
  $('#safetyView').innerHTML = `<section class="panel"><div class="panel-head"><div><h2>Safety reports</h2><p>CS can read. Manager/Admin can update investigation status.</p></div></div><div id="safetyTable">${simpleTable(data.items, [
    { label: 'Report', render: r => `<div class="row-title">${esc(r.report_type || 'Safety report')}</div><div class="row-sub">${esc((r.message || '').slice(0, 100))}</div>` },
...
function openSafetyAction(reportId, currentStatus) {
  ...
  $('#safetyForm').onsubmit = async event => {
    event.preventDefault();
    try {
      await request(`/ops/safety-reports/${encodeURIComponent(reportId)}`, { method: 'PATCH', body: JSON.stringify({ status: $('#safetyStatus').value, notes: $('#safetyNotes').value.trim() || null }) });
      $('#actionDialog').close(); toast('Safety report updated.'); await renderSafety();
    } catch (error) { toast(error.message, true); }
  };
}
...
function openAddStaff() {
  ...
  $('#addStaffForm').onsubmit = async event => {
    event.preventDefault();
    try {
      await request('/ops/staff', { method: 'POST', body: JSON.stringify({ user_id: $('#staffUserId').value.trim(), role: $('#staffRole').value, title: $('#staffTitle').value.trim() || null, reason: $('#staffReason').value.trim() }) });
      $('#actionDialog').close(); toast('Staff access granted.'); await renderStaff(true);
    } catch (error) { toast(error.message, true); }
  };
}
...
function openEditStaff(member) {
  ...
  $('#editStaffForm').onsubmit = async event => {
    event.preventDefault();
    try {
      await request(`/ops/staff/${encodeURIComponent(member.id)}`, { method: 'PATCH', body: JSON.stringify({ role: $('#editStaffRole').value, title: $('#editStaffTitle').value.trim() || null, enabled: $('#editStaffEnabled').value === 'true', reason: $('#editStaffReason').value.trim() }) });
      $('#actionDialog').close(); toast('Staff access updated.'); await renderStaff(true);
    } catch (error) { toast(error.message, true); }
  };
}
